...
int main(void)
{
	...
	// Start the scheduler
	xil_printf("All Tasks Complete, Starting the Scheduler\n");
	vTaskStartScheduler();
	...
	for(;;){
		if(xSemaphoreTake(xSemaphore_led,(portTickType)portMAX_DELAY) == pdTRUE){
			...
			if(uiLedFlag){
				...
				setLEDs(&GPIOInstance_Ptr, counter, 1);
				writeSPI(SPI_MODE_ENCODER_8, 0xDB, 0x01);

				if(counter % 15 == 0){
					xil_printf("Wrote 0xDB to SPI slave 0x01\n");
				}
			}else{
				// Set the GPIO Output to Low
				//setLEDs(&GPIOInstance_Ptr, LED_4 | LED_3, 0);
				setLEDs(&GPIOInstance_Ptr, counter, 0);
